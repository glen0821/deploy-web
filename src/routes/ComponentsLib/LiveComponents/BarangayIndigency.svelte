<script>
  ...
  const updateStatus = async (userID, selectedStatus, ownerID) => {
    ...
    if (selectedStatus == "Processing") {
      ...
      message =
        "Your indigency is ready to claim, Pick it up on your appointed date";
    } else {
          ...
          <tbody>
            {#each $onSnapsIndigency as barangayIndigency, i}
              ...
              <tr class="bg-white border-b">
                ...
                <td class="px-6 py-4">
                  <a href={barangayIndigency.validIDUrl}>
                    <img
                      src={barangayIndigency.validIDUrl}
                      alt="Valid ID"
                      class="h-10 w-10"
                    />
                  </a>
                ...
                {#if !$showPrintModel}
                  <td class="px-6 py-4">
                    <select
                      class="bg-white"
                      bind:value={barangayIndigency.gender}
                      on:change={() =>
                        updateGender(
                          barangayIndigency.id,
                          barangayIndigency.gender
                        )}
                    >
                    ...
                    <select
                      ...
                      on:change={() =>
                        updateStatus(
                          barangayIndigency.id,
                          barangayIndigency.status,
                          barangayIndigency.appointmentOwner
                        )}
                    >
                      ...
                      <button
                        ...
                        on:click={() => {
                          if (confirm('Are you sure you want to delete this item?')) {
                            removeData(barangayIndigency.id);
                          }
                        }}
